...
namespace UniCast.App.Services.Chat
{
    ...
    {
        // DÜZELTME: Artık HttpClient dışarıdan alınıyor veya factory'den
        ...
        /// <summary>
        /// HttpClient parametresi opsiyonel - verilmezse factory'den alınır.
        /// DI için test edilebilirlik sağlar.
        /// </summary>
        public YouTubeChatIngestor(HttpClient? http = null)
        {
            // DÜZELTME: Factory pattern kullanımı
        ...
        private async Task<string?> GetActiveLiveChatIdAsync(string apiKey, string videoId, CancellationToken ct)
        {
            ...
            foreach (var item in doc.RootElement.GetProperty("items").EnumerateArray())
            {
                if (item.TryGetProperty("liveStreamingDetails", out var lsd) && lsd.TryGetProperty("activeLiveChatId", out var cid))
            ...
            string apiKey, string liveChatId, string? pageToken, CancellationToken ct)
        {
            ...
            if (root.TryGetProperty("items", out var items))
            {
                foreach (var it in items.EnumerateArray())
                {
                    ...
                    if (it.TryGetProperty("authorDetails", out var ad) && ad.TryGetProperty("displayName", out var dn))
